...
def sync_actions(community_id: int):
    """Insert pressure widgets Bonde on petitions Action Network"""
    df = pd.read_sql_query(f'''
      ...
      WHERE c.id = {community_id} AND w.kind = 'pressure'
    ''', cnx)
    ...
    for index, item in df.iterrows():
        ...
        response = requests.post(
            "https://actionnetwork.org/api/v2/petitions/", data=json.dumps(payload), headers=headers)
